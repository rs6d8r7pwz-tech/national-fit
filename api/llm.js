...
// Codes renvoyés par le fournisseur quand il est temporairement surchargé :
// on retente automatiquement au lieu d'échouer.
...
export default async function handler(req, res) {
  ...
  let lastError = { status: 502, message: 'Erreur inconnue.' };

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    // Timeout par tentative pour ne pas bloquer indéfiniment.
    ...
    try {
      ...
      lastError = { status: upstream.status, message: errText };

      // Surcharge temporaire du fournisseur → on retente avec backoff.
      if (RETRYABLE.has(upstream.status) && attempt < MAX_ATTEMPTS) {
        await sleep(attempt * 1200); // 1.2s, puis 2.4s
        continue;
      }
      ...
      lastError = {
        status: 504,
        message: e.name === 'AbortError' ? 'timeout' : String(e),
      };
      // Erreur réseau/timeout → on retente aussi.
  ...
  const msg =
    lastError.message === 'timeout'
      ...
      : "Le service IA est momentanément surchargé. Réessaie dans quelques secondes.";
